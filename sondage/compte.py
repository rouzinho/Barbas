...
def tab_nb_occ(nom_fich):     #retourne un tableau des nombres d'occurences
	nb_occ = []
	f = open(nom_fich,'r')
	line = f.readline()
	while line != "":
...
def nb_total_occ(nom_fich):   #retourne le nombre d'occurences
	f = open(nom_fich,"r")
	t = f.readlines()
	l = len(t)
...
def tab_nom_occ(nom_fich):	#retourne le tableau des noms d'occurences
	nom_occ = []
	try :
		with open(nom_fich, 'r') as openedFile :
...
def max_occ(nom_fich):	#retourne le nombre total des choix de vote
	max_ = 0
	try :
		with open(nom_fich, 'r') as openedFile :
			for line in openedFile :
